...
namespace HighlightPlus {
    ...
    public partial class HighlightEffect : MonoBehaviour {
                ...
                if (rms[k].preserveOriginalMesh) {
                    ...
                    for (int l = 0; l < mesh.subMeshCount; l++) {
                        ...
                        if (_highlighted && ((outline > 0 && smoothOutlineVisibility != Visibility.Normal) || (glow > 0 && smoothGlowVisibility != Visibility.Normal) || (innerGlow > 0 && innerGlowVisibility != Visibility.Normal))) {
                            rms[k].fxMatMask[l].SetInt("_ZTest", (int)CompareFunction.Always);
                        } else {
                            rms[k].fxMatMask[l].SetInt("_ZTest", (int)CompareFunction.LessEqual);
                        }
            ...
            for (int k = 0; k < rmsCount; k++) {
                    ...
                    if (overlayColor.a > 0) {
                        for (int l = 0; l < mesh.subMeshCount; l++) {
                            ...
                            if (l < rms[k].fxMatOverlay.Length && rms[k].fxMatOverlay[l] != null) {
                                ...
                                fxMat.SetVector("_OverlayData", new Vector4(overlayAnimationSpeed, overlayMinIntensity, overlayBlending));
                ...
                for (int l = 0; l < mesh.subMeshCount; l++) {
                    ...
                    if (glow > 0 && glowQuality != QualityLevel.Highest) {
                        ...
                        mat.SetVector("_GlowDirection", Vector4.zero);
                        for (int j = 0; j < glowPasses.Length; j++) {
                            Color dataColor = glowPasses[j].color;
                            mat.SetColor("_GlowColor", dataColor);
        ...
        void UpdateMaterialPropertiesNow() {
            ...
            matOutline.SetInt("_Cull", cullBackFaces ? (int)CullMode.Back : (int)CullMode.Off);
            ...
            matGlow.SetInt("_Cull", cullBackFaces ? (int)CullMode.Back : (int)CullMode.Off);
            matGlow.SetFloat("_ConstantWidth", constantWidth ? 1.0f : 0);
            matGlow.SetInt("_GlowStencilOp", glowBlendPasses ? (int)StencilOp.Keep : (int)StencilOp.Replace);
        ...
        static readonly Dictionary<Mesh, Mesh> smoothMeshes = new Dictionary<Mesh, Mesh>();
        static readonly Dictionary<Mesh, Mesh> reorientedMeshes = new Dictionary<Mesh, Mesh>();
