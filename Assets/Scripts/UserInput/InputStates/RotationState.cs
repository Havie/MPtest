...
public class RotationState : InputState
{
    ...
    {
        if (_brain.InputDown() && _brain._currentSelection)
        {
            ...
            if (dis < _brain._holdLeniency && objWhereMouseIs == _brain._currentSelection)
            {
                ...
                if (_brain._pressTimeCURR > _brain._pressTimeMAX / 10) ///dont show this instantly 10%filled
                {
                    ...
                    _brain._currentSelection.ChangeMaterialColor(changeVal);
            ...
            else
            {
                ...
                _brain._currentSelection.ChangeMaterialColor(1);
            }
            ...
            if (_brain._pressTimeCURR >= _brain._pressTimeMAX)
            {
                ...
                _brain._currentSelection = _brain.FindAbsoluteParent(_brain._currentSelection);
                if (_brain._currentSelection)
                {
                    _brain._currentSelection.ChangeAppearanceMoving();
                    float zCoord = _brain._mainCamera.WorldToScreenPoint(_brain._currentSelection.transform.position).z;
                    _brain._mOffset = _brain._currentSelection.transform.position - _brain.GetInputWorldPos(zCoord);
                    _brain._objStartPos = _brain._currentSelection.transform.position;
                    _brain._objStartRot = _brain._currentSelection.transform.rotation;
                    ...
                    HandManager.PickUpItem(_brain._currentSelection); //might have moved to the wrong spot
                }
            ...
            else ///Do rotation
            {
                ...
                _brain._rotationAmount += _brain._currentSelection.DoRotation(rotation);
    ...
    private void HandleHighlightPreview()
    {
        ...
        HandManager.StartToHandleIntensityChange(_brain._currentSelection);

    }

    private void CancelHighLightPreview()
    {
        _brain._currentSelection.HandPreviewingMode = false;
